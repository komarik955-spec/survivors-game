...
function startDiscussion(roomId) {
  ...
  if (!state) return;
  console.log(`🎲 [${roomId}] startDiscussion раунд ${state.round}`);
  ...
  const roundEvent = state.roundEvent;
...
function finishVoting(roomId) {
  ...
  if (!gameOver) {
    ...
    state.phase = 'ended';
    setRoom(roomId, state);
    console.log(`🏁 [${roomId}] Игра окончена`);

    // Автоматический сброс через 10 секунд
    clearRoomTimer(roomId, 'autoResetTimeout');
    const timeout = setTimeout(() => resetGameToLobby(roomId), 10000);
...
function resetGameToLobby(roomId) {
  ...
  console.log(`🔄 [${roomId}] Автоматический сброс в лобби`);
